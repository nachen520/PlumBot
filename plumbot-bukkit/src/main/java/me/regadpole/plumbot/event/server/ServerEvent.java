...
public class ServerEvent implements Listener{
    ...
    public void onChat(AsyncPlayerChatEvent event) {
        ...
        if (Args.ForwardingMode() == 1) {
            ...
            for (long groupID : groups){
                PlumBot.getBot().sendMsg(true, "[小游戏]"+name+":"+fmsg,groupID);
            }
            return;
        }
        ...
        List<Long> groups = Config.getGroupQQs();
        for (long groupID : groups){
            PlumBot.getBot().sendMsg(true, "[小游戏]"+name+":"+message,groupID);
        }
    }
    ...
    public void onJoin(PlayerJoinEvent event){
        ...
        if (Config.WhiteList()) {
            PlumBot.getScheduler().runTaskAsynchronously(() -> {
                ...
                List<Long> groups = Config.getGroupQQs();
                for (long groupID : groups){
                    PlumBot.getBot().sendMsg(true, "玩家"+name+"前往小游戏区",groupID);
                }
            });
        } else {
            if (!Config.JoinAndLeave()){
    ...
    public void onQuit(PlayerQuitEvent event){
        ...
        List<Long> groups = Config.getGroupQQs();
        for (long groupID : groups){
            PlumBot.getBot().sendMsg(true, "玩家"+name+"离开小游戏区",groupID);
        }
    }
